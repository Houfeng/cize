...
const Context = new Class({
  ...
  ready: function (callback) {
    ...
    async.parallel([
      function (done) {
        mkdir(self.paths.out, done)
      },
      function (done) {
        mkdir(self.paths.cwd, done)
      }
    ], callback);
  },
  ...
  invoke: function (job, callback) {
    ...
    if (!TheJob) {
      return callback(new Error('Invalid Job'));
    }
    if (TheJob.project != self.project) {
      return callback(new Error('Can\'t cross project invoke in a context'));
    }
