...
const Store = new Class({
  ...
  insert: function (job, callback) {
    var self = this;
    callback = callback || utils.NOOP;
    return self.Record.find({ name: job.name },
      { offset: 0 }, 1, ['sn', 'Z'], function (err, records) {
        if (err) return callback(err);
        job.sn = records && records[0] ? records[0].sn + 1 : 0;
  ...
  _bindExitHandler: function () {
    ...
    exitHook(function () {
      ...
      if (fs.existsSync(self._exitPidPath)) {
        fs.appendFileSync(self._exitPidPath, pid);
      } else {
        fs.writeFileSync(self._exitPidPath, pid);
      }
    });
