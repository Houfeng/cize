...
const Project = new Class({
  ...
  job: function (jobName, runable) {
    ...
    if (!runable) {
      return this.jobs[jobName] || this.allJobs[name];
    }
  ...
  defineJob: function (name, runable) {
    ...
    if (!utils.isFunction(runableObj.runable)) {
      ChildJob = this.defineJob(utils.id(), runableObj.runable);
  ...
  invoke: function (job, params, callback) {
    ...
    if (!job) {
      return callback(new Error('Invalid parameter: job'));
    }
   ...
   * 调用前的钩子
   **/
  preInvoke: function (jobName, hook) {
    return this.job(jobName).preRun(hook);
  }
