...
const Project = new Class({
   ...
   * 定义一个新 Job 
   **/
  defineJob: function (name, runable, parent, nestedInvokeOptions) {
    if (!utils.checkName(name)) {
    ...
    if (!utils.isFunction(runableObj.runable)) {
      var ChildJob = this.defineJob(`${name}-child`, runableObj.runable, NewJob, nestedInvokeOptions);
      runableObj.runable = function (done) {
        return this.invoke(ChildJob, nestedInvokeOptions, this.done.bind(this));
      };
    }
