...
const Project = new Class({
  ...
  defineJob: function (name, runable) {
    ...
    if (!utils.isFunction(runableObj.runable)) {
      var ChildJob = this.defineJob(utils.newGuid(), runableObj.runable);
    ...
    NewJob.isVirtual = function () {
      return !(NewJob.project.job(NewJob.name));
    };
  ...
  invoke: function (job, params, callback) {
    if (!job) {
      throw new Error('Required parameters: job');
    }
