...
const Job = new Class({
  ...
  constructor: function (context, options) {
    ...
    this.stdout = this.parent && this.parent.stdout && options.stdout != 'new' ? this.parent.stdout : fs.createWriteStream(this.outFile);
  ...
  invoke: function (job, options, callback) {
    ...
    if (jobInfo.length > 1) {
      var childContext = this.context.create(jobInfo[0], this.params);
  ...
  _run: function (callback) {
    ...
    if (!self._canRun()) {
      self.refused = true;
      var err = new Error('Refused to run');
  ...
  _outAppendToParent: function (callback) {
    ...
    if (!self.parent ||
      !self.parent.stdout ||
      self.parent.stdout == self.stdout ||
      self.parent.stdout._writableState.finished) {
      return callback(self.error, self);
    }
    fs.readFile(self.outFile, function (err, data) {
      if (err) {
        self.parent.console.error(err.toString());
      }
      if (data) {
        self.parent.console.log(data.toString());
      }
      callback(self.error, this);
    });
  },
  ...
  _canRun: function () {
    ...
    return !self._beforeRunHooks.some(function (hook) {
      return utils.isFunction(hook) && hook.call(self, self) === false;
    });
  },
 ...
 * 定义子 Job
 **/
Job.defineJob = function (name, runable, nestedInvokeOptions) {
  return this.project.defineJob(name, runable, this, nestedInvokeOptions);
};
