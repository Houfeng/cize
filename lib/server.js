...
const Server = new Class({
  ...
  invoke: function (projectName, job, params, callback) {
    ...
    if (!utils.checkName(projectName)) {
      return callback(new Error('Invalid parameter: projectName'));
    }
    if (!job) {
   ...
   * 调用前的钩子
   **/
  preInvoke: function (projectName, jobName, hook) {
    return this.project(projectName).preInvoke(jobName, hook);
  }
