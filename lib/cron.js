...
if (process.env._CRON_SERVER) {
  ...
  function handle(data) {
    data.tasks.forEach(function (task) {
      var cronJob = new CronJob(task.expr, function () {
        request(`http://${LOCAL_HOST}:${data.port}/p/${task.projectName}/${task.jobName}/trigger?secret=${data.secret}`);
      });
      cronJob.start();
    });
  }
  ...
  function getCronData(server) {
    ...
    utils.each(server.projects, function (name, project) {
      utils.each(project.jobs, function (name, job) {
